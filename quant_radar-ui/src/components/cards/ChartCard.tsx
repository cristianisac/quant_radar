...
export function ChartCard({ card, height = 360, enlarged = false }: Props) {
  const ref = card.data_refs[0] ?? null;
  const { data, isLoading, error } = useDataRef(ref);
  const spec = card.chart_spec;

  const figure = useMemo(() => buildFigure(data ?? null, card, height), [
    data, card, height,
  ]);
  ...
  return (
    <div className="border border-border rounded-lg bg-panel p-3 h-full overflow-hidden flex flex-col">
      ...
      {error && (
        <div className="text-xs text-red-400">{String((error as Error).message)}</div>
      )}
      {data && figure && (
        <div className="flex-1 min-h-0">
          {/* eslint-disable-next-line @typescript-eslint/no-explicit-any */}
          <Plot
            data={figure.traces as any}
            layout={figure.layout as any}
            // plotly TS types reject the draw* modebar entries even
            // though runtime accepts them. Cast the config blob.
            config={{
              displayModeBar: enlarged,
              modeBarButtonsToAdd: enlarged
                ? (["drawline", "drawopenpath", "drawrect", "eraseshape"] as any)
                : [],
              scrollZoom: enlarged,
              responsive: true,
            }}
      ...
      {spec?.annotations && spec.annotations.length > 0 && (
        <div className="text-xs text-muted mt-1">
          {spec.annotations.length} saved annotation
          {spec.annotations.length > 1 ? "s" : ""}
        </div>
      )}
    </div>
  );
...
function buildFigure(
  ...
  // --- Annotations as additional traces / shapes ---
  const shapes: object[] = [];
  ...
  for (const sub of subplots) {
    ...
    else if (sub === "volume") y = (cols.volume ?? []).map((v) => v);
  ...
  // Domain calculation: top 60% for price, remaining 40% split across subplots
...
function addAnnotationToFigure(
  ...
  if (ann.kind === "hline" && ann.points.length > 0) {
    ...
    shapes.push({
      type: "line",
      x0: ann.points[0][0], x1: ann.points[0][0],
