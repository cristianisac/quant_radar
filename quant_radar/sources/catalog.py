...
CATALOG: dict[str, SourceCapability] = {
    ...
    "fmp": SourceCapability(
        ...
        kinds=["ohlcv"],
        intervals=["1m", "5m", "15m", "1h", "1d", "1w", "1mo"],
        history="Equities from listing date (typically 1985+ for major US tickers). Daily EOD reliable; intraday limited on free tier.",
        coverage="US equities + global ADRs + ETFs. ~40k tickers. Adapter wraps OpenBB Platform's `fmp` provider.",
        ...
        examples=["AAPL", "MSFT", "SPY", "TSLA", "NVDA"],
        schema={"ohlcv": ["open", "high", "low", "close", "volume"]},
        notes="Adapter goes through OpenBB Platform (obb.equity.price.historical). v3 REST endpoints deprecated; use stable/ if calling FMP directly.",
    ),
    "tiingo": SourceCapability(
        name="tiingo",
        kinds=["ohlcv"],
        intervals=["1d", "1h", "5m", "1m"],
        history="Equities from listing date. Daily EOD comprehensive; intraday via IEX free.",
        coverage="US equities + ETFs + select global ADRs. ~30k tickers. Adapter wraps OpenBB Platform's `tiingo` provider.",
    ...
    "polygon": SourceCapability(
        ...
        kinds=["ohlcv"],
        intervals=["1m", "5m", "15m", "1h", "1d", "1w", "1mo"],
        history="Free tier: ~2 years of historical EOD daily. Older data + intraday require paid tier.",
        coverage="US equities + ETFs + indices + crypto + FX. ~70k tickers. Hand-written REST adapter (Polygon is not in OpenBB Platform's bundled providers).",
        ...
        schema={"ohlcv": ["open", "high", "low", "close", "volume"]},
        notes="REST aggregates endpoint: /v2/aggs/ticker/<SYM>/range/<mult>/<span>/<from>/<to>. Adapter pages results via limit=50000.",
    ),
